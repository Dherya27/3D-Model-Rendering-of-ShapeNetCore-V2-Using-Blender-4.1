...
# lamp = bpy.data.lights.new(name="Sunlight", type="SUN")
# lamp.shadow_soft_size = 0
# # Possibly disable specular shading:
# # lamp.specular_intensity = 0

# # Add another light source so stuff facing away from light is not completely dark
# light_data = bpy.data.lights.new(name="light_2", type='AREA')
...
# Add the lamp object to the scene
lamp_obj = bpy.data.objects.new(name="Sunlight", object_data=lamp)
...
light2_obj = bpy.data.objects.new(name="light_2", object_data=light_data)
bpy.context.collection.objects.link(light2_obj)

# Set the newly added light as the active object
bpy.context.view_layer.objects.active = light2_obj

# Add another light source
bpy.ops.object.light_add(type='SUN')
lamp2 = bpy.data.lights['Sun']
lamp2.shadow_soft_size = 0  # Disable shadows
lamp2.color = (0, 0, 0)  # Set color to black to effectively disable specular shading
lamp2.energy = 0.5

# Rotate the second sunlight by 180 degrees along the X-axis
bpy.data.objects['Sun'].rotation_euler[0] += 180
...
rotation_mode = 'XYZ'  # Define the rotation mode here

# Set up camera rotation mode
cam.rotation_mode = rotation_mode

# Calculate step size for rotating the camera
stepsize = 360 / args.views  # Assuming args.views is defined elsewhere
